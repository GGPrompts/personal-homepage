        ...
        <TooltipTrigger asChild>
          ...
          >
            <div className={`flex items-center gap-3 transition-all duration-300 ${collapsed && !mobile ? 'justify-center' : ''}`}>
              <div className="h-10 w-10 rounded-lg bg-primary/20 flex items-center justify-center flex-shrink-0 transition-colors duration-200 group-hover:bg-primary/30">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <PopoverTrigger asChild>
                          <button
                            className={`
                              w-full flex items-center justify-center p-2.5 rounded-lg transition-colors duration-200 group
                              ${hasActiveSection
                          ...
                          return (
                            <li key={item.id}>
                              <button
                                ...
                                className={`
                                  w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-200 group
                                  ${isActive
                                    ? 'glass text-primary border-glow'
                                    : 'hover:bg-primary/10 text-muted-foreground hover:text-foreground'
                                  }
              ...
              return (
                <div key={category.id} data-tabz-category={category.id}>
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors duration-200 group rounded-md hover:bg-primary/5"
                      ...
                      return (
                        <li key={item.id}>
                          <button
                            ...
                            className={`
                              w-full flex items-center gap-3 px-3 py-2 pl-5 rounded-lg transition-colors duration-200 group relative
                              ${isActive
                                ? 'glass text-primary border-glow'
                                : 'hover:bg-primary/10 text-muted-foreground hover:text-foreground'
                              }
                          ...
                          >
                            ...
                            <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] rounded-r-full bg-primary transition-opacity duration-200 ${isActive ? 'h-[60%] opacity-100' : 'h-0 opacity-0'}`} />
                            <div className="relative flex-shrink-0">
                              <Icon className={`h-4 w-4 ${showNowPlaying && isPlaying ? 'animate-pulse' : ''}`} />
                              {item.id === "jobs" && jobsNeedsHumanCount > 0 && (
                                <span className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-amber-500 text-[9px] flex items-center justify-center text-white font-medium animate-pulse">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  {/* Active indicator bar */}
                  {!(collapsed && !mobile) && (
                    <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] rounded-r-full bg-primary transition-opacity duration-200 ${activeSection === "settings" ? 'h-[60%] opacity-100' : 'h-0 opacity-0'}`} />
                  )}
